...
function copyComponents() {
  // 清空docs/components目录下所有非index.md文件
  getAllFilesInDirectory('./docs/components/').forEach((el) => {
    if (!/index\.md$/.test(el)) {
      fs.unlinkSync(el)
    }
  })

  const files = getAllFilesInDirectory('../components/src/').filter((item) => /__demo__.*\.md$/.test(item))
  ...
  for (const filePath of files) {
    ...
    fs.copyFileSync(filePath, `./docs/components/${path.basename(filePath)}`)
  }
  ...
  `
    fs.writeFileSync('./docs/.vitepress/configs/sidebar/components.js', content, {
      encoding: 'utf8'
    })
  }
}
